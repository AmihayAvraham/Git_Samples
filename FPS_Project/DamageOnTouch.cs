...
namespace MoreMountains.TopDownEngine
{
    ...
    public class DamageOnTouch : MonoBehaviour
    {
        ...
        protected virtual void OnCollideWithDamageable(Health health)
        {
            ...
            //MoveBehaviour controller = health.gameObject.MMGetComponentNoAlloc<MoveBehaviour>();
            AICharacter aiChar = health.gameObject.MMGetComponentNoAlloc<AICharacter>();
            _colliderRigidBody = health.gameObject.MMGetComponentNoAlloc<Rigidbody>();

            if (aiChar != null && (DamageCausedKnockbackForce != Vector3.zero) && (!_colliderHealth.Invulnerable) && (!_colliderHealth.ImmuneToKnockback))
            {
                ...
                else // if we're in 3D
                {
                    if (DamageCausedKnockbackDirection == KnockbackDirections.BasedOnSpeed)
                    {
                        ...
                        _knockbackForce = DamageCausedKnockbackForce * _velocity.magnitude;
                    }
                    if (DamageCausedKnockbackDirection == KnockbackDirections.BasedOnOwnerPosition)
                    {
                        if (Owner == null) { Owner = this.gameObject; }
                        //Vector3 relativePosition = _colliderTopDownController.transform.position - Owner.transform.position;
                        Vector3 relativePosition = health.gameObject.transform.position - Owner.transform.position;
                ...
                {
                    // Add fallback force
                    aiChar.AddImpact(_knockbackForce.normalized, _knockbackForce.magnitude);
            ...
            if (Owner != null && Owner.tag !=  health.gameObject.tag)
                _colliderHealth.Damage(DamageCaused, gameObject, InvincibilityDuration, InvincibilityDuration, _damageDirection, DamageType);
        ...
        protected virtual void SelfDamage(int damage)
        {
            if (_health != null)
            {
                ...
                _health.Damage(damage, gameObject, 0f, DamageTakenInvincibilityDuration, _damageDirection, DamageType.Normal);
            }
