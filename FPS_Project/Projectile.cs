...
{	
	...
	public class Projectile : MMPoolableObject  
	{
		...
		[SerializeField]
		private bool spawnHit;
		[SerializeField]
		private MMObjectPooler hitObjectPooler;

		public ProjectileMover projectileVfx;
        ...
        protected virtual void FixedUpdate ()
		{
			...
			if (Time.time - initialTime >= durationActive)
            {
				Kill();
			}
		}
		...
		ParticleSystem flashObj;
		/// <summary>
		/// On death, we stop our projectile
		/// </summary>
		protected virtual void OnHit(GameObject hitter)
		{
			if (spawnHit)
			{
				pooledObj = hitObjectPooler.GetPooledGameObject();
				pooledObj.transform.position = this.transform.position;
				pooledObj.SetActive(true);
				/*flashObj = pooledObj.GetComponent<ParticleSystem>();
				if (flashObj != null)
					flashObj.Play();*/
				//projectileVfx.OnCollision(hitter.transform);
			}
		}
		...
		/// On death, we stop our projectile
		/// </summary>
		protected virtual void OnDeath(DamageType damageType)
		{
			StopAt ();
		}
